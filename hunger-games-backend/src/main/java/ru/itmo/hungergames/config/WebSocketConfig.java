...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
        ...
        registration.interceptors(new ChannelInterceptor() {
            ...
            public Message<?> preSend(Message<?> message, MessageChannel channel) {
                ...
                if (StompCommand.CONNECT.equals(accessor.getCommand())) {
                    accessor.setUser(createAuthenticationFromJwtToken(parseJwt(accessor).get()));
                }
                return message;
            }
        });
    ...
    private Optional<String> parseJwt(StompHeaderAccessor accessor) {
        List<String> authorization = accessor.getNativeHeader("passcode");
        log.info("passcode: {}", authorization);
        String headerAuth = authorization.get(0).split(" ")[1];
